...
namespace W8.WPFDevToolkit.UC.PopupControl.Lib
{
    public class PopupHelper
    {
        ...
        {
            return CloseAsync(PopupCloseAction.CloseCalled);
        }

        protected async Task CloseAsync(PopupCloseAction action)
        {
        ...
        void Overlay_Tapped(object sender, Windows.UI.Xaml.Input.TappedRoutedEventArgs e)
        {
            ...
            if (m_popup.IsOpen)
            {
                var dummy = CloseAsync(PopupCloseAction.OverlayDismissal);
            }
        }
